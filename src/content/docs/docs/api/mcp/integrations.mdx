...
<CardGrid>
  ...
  <LinkCard
    title="Claude.ai"
    description="Web interface with native OAuth MCP support"
...
- **For interactive compliance queries** → Claude.ai, Claude Desktop
...
You: "Add a new third party called CloudBackup Inc that provides backup services"
Claude: [Uses addThirdParty tool]
...
```python
# Type: "Create a function to check if all third parties have been reviewed"
# Cursor generates:

async def check_third_party_reviews(organization_id):
    """Check if all third parties have recent reviews."""
    thirdParties = await probo.list_third_parties(organization_id)

    overdue = []
    for third_party in thirdParties:
        days_since_review = (datetime.now() - third_party.last_reviewed).days
        if days_since_review > 90:
            overdue.append(third_party)
...
claude-code "List all third parties that haven't been reviewed in 90 days"
```
...
2. **Third-Party Management**: List third parties, check reviews, send reminders
...
class ProboClient {
  ...
  async checkThirdPartyCompliance(orgId: string) {
    // List all third parties
    // Check review dates
    // Return overdue third parties
  }
}
```
