  ...
  "mcpServers": {
    "probo": {
      ...
      "metadata": {
        ...
        "description": "Access compliance data and manage risks, third parties, and audits"
      }
    }
  }
}
...
def generate_compliance_report(organization_id):
    ...
    thirdParties = fetch_third_parties(organization_id)
...
```
You: "Write a Node.js script to check all third parties have been reviewed in the last 90 days"
...
async def daily_compliance_check():
    ...
    thirdParties = await client.list_third_parties(organization_id)

    # Analysis and reporting logic
    report = generate_report(risks, obligations, thirdParties)
...
export class ProboAPIClient {
  ...
  async addThirdParty(input: AddThirdPartyInput): Promise<ThirdParty> {
    // Implementation
  }
...
- "Check third-party review status"
