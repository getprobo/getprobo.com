...
1. Data breach from third-party systems (Risk Score: 20)
...
```
You: "Create a third party risk assessment document for CloudStorage Inc"

Cursor:
[Fetches third-party details from Probo]
...
```javascript
// Cmd+K: "Create a function to check if all critical third parties have recent reviews"

// Cursor generates:
async function checkThirdPartyReviews() {
  // Uses Probo MCP listThirdParties tool
  // Checks last review dates
  // Returns third parties needing review
}
```
...
```json
{
  ...
  "cursor.chat.welcomeMessage": "I can help with compliance management using Probo. Try: 'List all third parties' or 'Show high-priority risks'"
}
```
...
class ProboClient {
  ...
  async addThirdParty(data: AddThirdPartyInput): Promise<ThirdParty> {
    // Implementation
  }
}
```
...
class TestComplianceData(unittest.TestCase):
    def test_all_third_parties_have_reviews(self):
...
// In Cursor Settings → Snippets
{
  ...
  "Third-Party Review Status": {
    "prefix": "@third-parties-review",
    "body": "List all third parties and their last review dates from Probo"
  }
}
```
