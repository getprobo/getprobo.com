...
probod:
  ...
  trust-center:
    http-addr: ":80"
    https-addr: ":443"
    base-domain: "probopage.example.com"
...
### Compliance Portal Configuration

The YAML key remains `trust-center` for backward compatibility, but this block configures the **Compliance Portal** server.

#### `trust-center.http-addr` (string)

**Default**: unset

Network address and port where the compliance portal HTTP server listens (for example ACME HTTP-01 challenges and HTTP to HTTPS redirects). Environment variable: `PROBOD_TRUST_CENTER_HTTP_ADDR`.

#### `trust-center.https-addr` (string)

**Default**: unset

Network address and port where the compliance portal HTTPS server listens for secure connections and custom-domain routing. Environment variable: `PROBOD_TRUST_CENTER_HTTPS_ADDR`.

#### `trust-center.base-domain` (string)

**Default**: unset

Base domain used for compliance portal hosting. Environment variable: `PROBOD_TRUST_CENTER_BASE_DOMAIN`.
