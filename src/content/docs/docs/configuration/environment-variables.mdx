...
| Variable                    | Description                                            | Default Value | Required |
| --------------------------- | ------------------------------------------------------ | ------------- | -------- |
| `OAUTH2_SERVER_SIGNING_KEY` | PEM-encoded RSA private key used to sign OAuth2 tokens | -             | **Yes**  |
...
5. `OAUTH2_SERVER_SIGNING_KEY` - Generate as a PEM RSA private key
...
export OAUTH2_SERVER_SIGNING_KEY="$(openssl genpkey -algorithm RSA -pkeyopt rsa_keygen_bits:2048)"
...
echo "OAUTH2_SERVER_SIGNING_KEY is set (PEM key hidden)"
```
