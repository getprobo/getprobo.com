...
- Use stable Probo IDs in workflow data; treat IDs and pagination cursors as
  opaque strings.
- Make downstream effects idempotent. Probo retries transient webhook failures
  with the same `eventId`, and n8n executions can also be retried manually or
  by workflow error handling.
