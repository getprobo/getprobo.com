...
- Record `eventId` after verification and process it only once. Retries reuse the same `eventId` with a new timestamp and signature. Timestamp validation limits replay time; idempotency prevents duplicate side effects.
