...
- Keep a separate secret for each subscription and store it in a secret manager. The secret is issued when the subscription is created and changes only if you delete that subscription and create a new one. See [Signing secret](/docs/developers/api/webhooks/overview#signing-secret).
- Return a generic `400` or `403` response. Do not reveal which verification check failed.
- Record `eventId` after verification and process it only once. The same value is sent as `Idempotency-Key` and `X-Probo-Webhook-Delivery-Id`. Retries reuse it with a new timestamp and signature. Timestamp validation limits replay time; idempotency prevents duplicate side effects.
