---
title: Cloudflare
description: Connect Cloudflare as an access source using a scoped API Token
---

Probo reads your Cloudflare account's members through the Cloudflare API so you can review who has access.

:::caution
Use a scoped **API Token** (an opaque string with no prefix) created in the Cloudflare dashboard under **My Profile** > **API Tokens**. A **Global API Key** will **not** work, because it authenticates with `X-Auth-Email` and `X-Auth-Key` headers instead of the `Authorization: Bearer` scheme Probo uses.
:::
...
![Creating a custom API token in the Cloudflare dashboard](/docs/access-review/cloudflare-create-api-key.webp)
