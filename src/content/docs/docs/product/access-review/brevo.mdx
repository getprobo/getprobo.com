---
title: Brevo
description: Connect Brevo as an access source using an API key
---
...
{/* Screenshot pending: save the key-creation dialog to public/docs/access-review/brevo-create-api-key.webp and restore the image line here. */}
...
- **Calls stop working after the key worked once.** Brevo can block API calls from unauthorized IP addresses. Under **Settings** > **Security** > **Authorized IPs**, check whether blocking is active for API keys, then authorize the addresses Probo calls from or deactivate the blocking.
