...
- **Calls stop working after the key worked once.** Brevo can block API calls from unauthorized IP addresses. Under **Settings** > **Security** > **Authorized IPs**, allow the [Probo Cloud egress address](/docs/deployment/ip-ranges) for your region, or disable API-key IP blocking. For a self-hosted deployment, authorize that deployment's fixed egress addresses.
