...
:::caution
Use a **Cursor Admin API key** (`crsr_…`) created by a team administrator in the Cursor dashboard. A personal **User API Key** will **not** work, because it is not a team-admin key and cannot list team members through the Admin API.
:::
...
3. Copy the key (`crsr_…`) and store it securely. It's shown only once.
...
- **Key rejected.** Confirm it's a Cursor Admin API key (`crsr_…`) created by a team administrator. A personal User API Key won't work, and because the Admin API accepts only Basic auth, Bearer tokens are rejected.
