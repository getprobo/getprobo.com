...
2. Find **Resend**, click the arrow next to **OAuth** and choose **API Key**, paste the key, and click **Connect**. Probo registers the Resend OAuth client on every deployment, so **API Key** is always in that menu.
