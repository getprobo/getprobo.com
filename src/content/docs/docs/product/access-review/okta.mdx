...
3. For Probo Cloud, restrict the token to the [egress IP](/docs/deployment/ip-ranges) for your region, or set **API calls made with this token must originate from** to **Any IP**. For a self-hosted deployment, allowlist that deployment's fixed egress addresses in an Okta network zone. Click **Create token**, then copy the token value and store it securely. Okta shows it only once.
...
- **No users appear.** The token carries the permissions of the admin who created it, so it must come from an admin who can view Users. For Probo Cloud, the token's network condition must allow the [Cloud egress IP](/docs/deployment/ip-ranges) for your region, or **Any IP**. For a self-hosted deployment, it can instead allowlist that deployment's fixed egress addresses.
