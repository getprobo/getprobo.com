---
title: Okta
description: Connect Okta as an access source using an Okta API token
---
...
{/* Screenshot pending: save the key-creation dialog to public/docs/access-review/okta-create-api-key.webp and restore the image line here. */}
...
3. Set **API calls made with this token must originate from** to **Any IP** unless you allowlist Probo in an Okta network zone. Click **Create token**, then copy the token value and store it securely. Okta shows it only once.
...
- **No users appear.** The token carries the permissions of the admin who created it, so it must come from an admin who can view Users. Also check that the token is not restricted to a network zone that excludes Probo. The creation dialog can limit calls to specific IP addresses or zones.
