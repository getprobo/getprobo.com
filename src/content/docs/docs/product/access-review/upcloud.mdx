...
5. For Probo Cloud, you can restrict the token to the [egress IP](/docs/deployment/ip-ranges) for your region, or keep **Allow access from all IP addresses** checked. For a self-hosted deployment, restrict the token to that deployment's fixed egress addresses.
...
- **Token rejected.** Confirm the token starts with `ucat_` and has not expired. If you restricted the token to specific IPs, include the [Cloud egress address](/docs/deployment/ip-ranges) for your region. A self-hosted deployment can use its own fixed egress addresses. Your UpCloud username and password won't work here.
