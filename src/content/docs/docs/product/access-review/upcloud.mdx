---
title: UpCloud
description: Connect UpCloud as an access source using a main-account API token
---
...
5. Keep **Allow access from all IP addresses** checked, or restrict it to the egress addresses your Probo deployment uses.
...
- **Token rejected.** Confirm the token starts with `ucat_` and has not expired. If you restricted it by IP, the restriction must cover your Probo deployment. Your UpCloud username and password won't work here.
