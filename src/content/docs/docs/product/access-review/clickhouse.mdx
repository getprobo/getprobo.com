...
- **Requests fail from Probo but the key works locally.** If the key was created with **Specific locations** under **Allow access to this API Key**, include the [Probo Cloud egress address](/docs/deployment/ip-ranges) for your region, or remove the restriction. For a self-hosted deployment, allowlist that deployment's fixed egress addresses.
