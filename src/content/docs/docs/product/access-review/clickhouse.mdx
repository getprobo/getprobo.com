...
description: Connect ClickHouse Cloud as an access source using a Cloud API key ID and secret
---
...
{/* Screenshot pending: save the key-creation dialog to public/docs/access-review/clickhouse-create-api-key.webp and restore the image line here. */}
...
- **Requests fail from Probo but the key works locally.** If the key was created with **Specific locations** under **Allow access to this API Key**, only the IP addresses you listed can use it. Remove the restriction, or allowlist the addresses Probo connects from.
