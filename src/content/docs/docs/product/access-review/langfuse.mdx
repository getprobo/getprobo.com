...
- A Langfuse plan that includes the organization management API, which Langfuse gates behind the `admin-api` entitlement. Langfuse grants that entitlement to Langfuse Cloud **Enterprise**, to **Pro with the Teams add-on** (the plan Langfuse records internally as `Team`), and to a self-hosted **Enterprise Edition** deployment. Plain Hobby, Core and Pro do not have it, and neither the open source nor the self-hosted Pro edition does. Note that Langfuse's pricing page lists Hobby, Core, Pro and Enterprise only: the qualifying middle option is the Teams add-on bought on top of Pro, not a separate tier you can pick. If your organization is on a legacy or individually negotiated plan, confirm the entitlement with Langfuse. Without it Langfuse hides the **API Keys** tab and the memberships endpoint answers `This feature is not available on your current plan.`
...
![Creating an organization-scoped API key in Langfuse organization settings](/docs/access-review/langfuse-create-api-key.webp)
