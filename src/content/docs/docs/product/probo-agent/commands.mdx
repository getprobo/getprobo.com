...
description: Command reference for the probo-agent CLI
---
...
## Browser enrollment notes

- Enrollment deeplinks use `probo://enroll?server=...&token=...`.
- On macOS, browser enrollment is handled by the signed **Probo Agent.app**; CLI elevation from the deeplink is refused.
- On Windows, tray-assisted elevated install is available after the agent is present.
- Linux and FreeBSD enroll through `probo-agent install` or `install.sh`.
