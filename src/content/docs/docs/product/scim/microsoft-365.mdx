...
description: Configure direct SCIM provisioning from Microsoft Entra ID so Probo accounts are created, updated, and deactivated automatically as users change.
---
...
Assigning a role and employment type to each Microsoft Entra ID user lets Probo attribute the right policies to the right people and run meaningful access reviews. Without these, a user shows up in Probo but can't be properly evaluated for least-privilege access.

Microsoft Entra ID syncs two relevant fields to Probo via the SCIM bridge:

- **Job title** → the user's role (for example, Software Engineer, Finance Manager, IT Administrator)
- **Employee type** → their employment type (for example, Full-time, Part-time, Intern, Contractor, Freelance)
...
4. Set the **Job title** field to the user's role
5. Set the **Employee type** field to their employment type
6. Click **Save**
