...
| `<probo-banner>`             | Container for the first-layer banner card. Visibility follows `layout.initial_state` (closed in every opt-out jurisdiction — see [Settings link](#settings-link)).                                             |
...
| `<probo-acknowledge-button>` | Wraps a button that records `ACKNOWLEDGE` for **NOTICE** presentations (informational dismiss, currently Mexico only). Do not reuse accept-all for this.                                                       |
...
| ------------ | ------------------- | ----------- | --------------------- | ----------- |
| **OPT_OUT** (CCPA) | CCPA / CPRA, and US visitors whose state cannot be resolved | Always replaced with the statutory **“Your Privacy Choices”** text and [official opt-out icon](https://www.law.cornell.edu/regulations/california/11-CCR-7015) (English; not translated) | Closed by default | **Privacy Choices** panel (`privacy_choices`) |
...
Most visitors fall under `OPT_OUT`, where nothing opens on its own. In those jurisdictions the settings link is not a convenience — it is the visitor's only route to their choices, so it must be present on every page. See [Geolocation and regulations](/docs/product/cookie-banner/geolocation) for the full mapping.
