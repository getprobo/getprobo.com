...
See the dedicated guide: [How to set up PostHog: GDPR, CCPA, and global privacy laws](/blog/2026-05-27-posthog-cookie-banner-gdpr-ccpa-compliance). It covers both the cookieless-only setup and the consent-aware setup (full PostHog features behind a regulation-aware banner), with a minimum working example for each.

A complete working integration lives in the [`cookie-banner-react`](https://github.com/getprobo/probo/tree/main/examples/cookie-banner-react) example.
