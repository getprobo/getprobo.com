...
1. **Create a banner in the Probo console**
   ...
   | **Consent Mode** | Choose **Opt-in** (recommended for GDPR) or **Opt-out**. This is the default for visitors in countries with no specific regulation — Probo automatically overrides it based on the visitor's location when a regulation applies. See [Geolocation and Regulations](/docs/product/cookie-banner/geolocation). |
