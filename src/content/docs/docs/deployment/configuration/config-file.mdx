...
The Compliance Portal `tls-mode` accepts `direct` or `external` and defaults to
`direct`. Direct mode serves portal TLS itself from `https-addr`. External mode
expects a trusted layer-7 proxy to terminate TLS and forward requests to
`http-addr`; see [Compliance Portal TLS](/docs/deployment/configuration/environment-variables#compliance-portal-tls) for the required routing and header controls.
