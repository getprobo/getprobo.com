...
1. **Clone the repository:**
   ...
   The `compose.prod.yaml` file and the supporting `compose/` directory (with database init scripts and storage config) are all needed and are included in the repository.
...
3. **Create environment file:**
   ...
   PROBOD_API_CORS_ALLOWED_ORIGINS=https://your-domain.com

   # Email settings (configure your SMTP)
   PROBOD_SMTP_ADDR=smtp.example.com:587
   PROBOD_SMTP_USER=your-smtp-username
   PROBOD_SMTP_PASSWORD=your-smtp-password
   PROBOD_SMTP_TLS_REQUIRED=true
   PROBOD_MAILER_SENDER_EMAIL=no-reply@your-domain.com
   EOF
   ```
