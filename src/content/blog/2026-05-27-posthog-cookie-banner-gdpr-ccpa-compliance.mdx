...
date: 2026-05-27
excerpt: "Two clean ways to wire PostHog into a website that respects GDPR, CCPA, UK GDPR, LGPD, PIPEDA, and the rest of the alphabet soup of privacy laws — without losing analytics."
author:
  name: "Émile Ré"
faqs:
  ...
  - question: "Why does cookieless mode break session replay and feature flags?"
    answer: "Both rely on persistent browser storage that cookieless mode forbids. Session replay needs a session ID kept across page loads. Feature flag caching needs to store the last resolved flag values. Without storage, replays can't be captured at all and feature flags must be re-fetched on every page load (or bootstrapped server-side). You also lose identify() because a stable distinct_id would qualify as personal data under GDPR."
