<div class={className} bind:this={intersection.ref}>
    {#each visibleFrameworks as framework, index (index)}
        ...
        >
            ...
            >
                <FrameworkBadge name={framework.badge} class="block size-16 mx-auto"/>
...
<script lang="ts">
  ...
  const duration = 5000;
  let {count, class: className, countMobile}: { count: number, countMobile?: number, class: string } = $props();
